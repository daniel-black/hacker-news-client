...
const Item = (props: ItemProps) => {
    ...
    return (
      <div className="space-y-3">
        <h3 className="font-bold leading-tight whitespace-normal font-mono flex items-center">
          ...
          <Link href={`/item/${props.parent}`}>
            <>
              (Replying to <span className="text-pink-600">PARENT</span> post)
            </>
          </Link>
        </h3>
        <div
          className="whitespace-normal border-l-2 pl-2 ml-2 border-pink-500"
    ...
    return (
      <div className="pr-2">
        ...
        {isItemPage && props.text ? (
          <>
            <button
              className="text-slate-600 underline pl-3 text-sm"
              onClick={() => setDisplayText(!displayText)}
            >
              {displayText ? "Hide" : "Show"} text
            </button>
            <div
              className={displayText ? "px-3 whitespace-normal" : "hidden"}
              suppressHydrationWarning
              dangerouslySetInnerHTML={{ __html: props.text }}
            ></div>
          </>
  ...
  const renderTitle = () => {
    ...
    if (url) {
      ...
      return (
        <div>
          ...
          <a className="story-url" href={url}>
            ({shortUrl})
          </a>
        </div>
      );
    ...
    return (
      <Link href={`/item/${id}`} className="item-title">
        <>{title}</>
      </Link>
    );
  };
  ...
  const renderScore = () => (
    <span className="text-slate-500">🔼{score || 0}</span>
  );

  const renderDiscussionLink = () =>
    props.descendants ? (
      <Link href={`/item/${id}`} className="comments-link" title="Comments">
        <>🗨️{props.descendants}</>
      </Link>
    ) : (
  ...
  const renderUserId = () => (
    <Link href={`/user/${by}`} className="user-link">
      <>👤{by}</>
    </Link>
  );
